...
export default d3Piece({
  ...
  // Hue is the ONLY place a scatter's series name appears — there is no axis
  // tick and no direct label to carry it, unlike a bar's categories or a
  // strip's rows. Without this the piece drew four colours and named none of
  // them. Built with the same assignColors() call the draw makes, so the
  // swatch cannot drift from the dot.
  key(stats, state, { colors, accent }) {
    const names = stats.seriesNames || [];
    if (names.length < 2) return null;
    const map = assignColors(names, colors, { accent });
    return names.map((name) => ({ name, color: map.get(name) }));
  },
