...
export default d3Piece({
  ...
  // Only when a series is bound. With no split the axis already names every
  // bar and hue encodes nothing, so a key there would be a swatch beside a
  // label the reader is already reading. With a split, the group or stack
  // member's name appears nowhere but the colour — which is what this fixes.
  key(stats, state, { colors, accent }) {
    const names = stats.seriesNames || [];
    if (names.length < 2) return null;
    const map = catColors(stats, colors, accent);
