...
export function mount(el, config = {}) {
  ...
  function draw(width, height) {
    ...
    // The gutter has to clear the WIDER of the two lines written into it. The
    // group name sits above an `n=… · med …` line that is routinely longer —
    // "Chinstrap" is nine characters, its own summary is sixteen — and sizing
    // this from the name alone pushed that second line back past x=0, where the
    // viewBox cut its leading characters off with nothing to show it had.
    const subLabel = (g) => `n=${g.n} · med ${formatNumber(g.median)}`;
    const labelRoom = Math.min(150, Math.max(64, ...data.map((g) => Math.max(
      g.label.length * 7,      // 11.5px label face
      subLabel(g).length * 6,  // 9.5px tabular mono
    ) + 18)));
    ...
    data.forEach((group, gi) => {
      ...
      nLabel.textContent = subLabel(group);
