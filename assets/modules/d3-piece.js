...
//
// The 32 animated pieces in the House gallery are hand-written D3 over their
...
import {
  buildFrame, observeSize, resolveCategories, resolveAccent, createMotion, dataChanged,
...
export function d3Piece(spec) {
  ...
  function mount(el, config = {}) {
    ...
    function frameOpts() {
      return {
        // A KEY IS NOT DECORATION WHEN HUE IS THE ONLY THING CARRYING A NAME.
        //
        // Most ported forms label their marks directly — a sankey names its
        // nodes, a treemap its tiles, a strip its rows — and a legend beside
        // those is redundant clutter, which is why `false` was the blanket
        // default. But two forms encode a series in hue and put that name
        // NOWHERE else: a grouped/stacked bar, and a scatter coloured by a
        // category. Both drew three or four hues with nothing to read them by.
        // A spec that declares `key` gets the frame's legend; everything else
        // is unchanged.
        legend: Boolean(spec.key),
    ...
    /**
     * Fills the frame's key from the spec's own colour map.
     *
     * `spec.key` is handed the SAME `colors` array the draw was handed and is
     * expected to build its map with the SAME call the draw makes
     * (catColors(), assignColors()) — so the swatch cannot drift from the mark
     * it stands for. A key rebuilt independently is a key that is right until
     * someone changes the assignment on one side only.
     *
     * Redrawn after every render because the palette is resolved from computed
     * CSS, so a theme swap on the host page changes it under us.
     */
    function paintKey() {
      if (!frame.legend || !spec.key) return;
      let items = [];
      try {
        items = spec.key(state.stats, state, { colors, accent: resolveAccent(el) }) || [];
      } catch { items = []; }
      frame.legend.textContent = '';
      // Nothing to distinguish is not a one-item key, it is no key. A single
      // swatch beside a single-series chart is pure noise.
      if (items.length < 2) { frame.legend.style.display = 'none'; return; }
      frame.legend.style.display = '';
      for (const item of items) {
        const li = document.createElement('li');
        const span = document.createElement('span');
        span.className = 'vf-swatch';
        span.style.background = item.color;
        // The swatch is the colour; the name beside it is what a screen reader
