...
//
// THE ANIMATED HOUSE GALLERY, AS A LIBRARY YOU CAN BROWSE.
//
...
const LIMITS = {
  ...
  bumpPeriods: 24,    // columns a bump can separate — beyond this crossings merge
};
...
function edgePair(p) {
  ...
  // A ribbon's width is a SUM over its edge, so the weight is chosen on the
  // same terms as any other area column — see sizeColumn. Taking quants()[0]
  // here weighted the penguin flow by total bill length.
  //
  // And where NOTHING on offer is summable, this returns no weight at all
  // rather than sizeColumn's best-available fallback. That fallback is right
  // for a form that can shrug and draw something; it is wrong here, because a
  // ribbon's width is a quantity claim: an arc diagram weighted by the added-up
  // "speed" of eighteen models states a volume that does not exist. The forms
  // downstream already handle an absent weight — the arc draws unweighted, the
  // sankey and the chord refuse in their own words.
  const exclude = pair.map((f) => f.name);
  const summable = quants(p, exclude).find((f) => !perItemMeasure(f)) || null;
  return { source: pair[0], target: pair[1], value: summable };
}
...
/**
 * A DIMENSIONAL MEASUREMENT, spotted by its unit. `bill_length_mm` carries none
 * of the words above, so it used to clear RATE_NAME and get summed: the bar
 * chart over penguins.csv announced "Adelie takes 39% of the 15k
 * bill_length_mm", which is the added-up bill length of 151 birds and a
 * quantity that exists nowhere.
 *
 * The line drawn here is whether adding two of them yields a third of the same
 * kind. Mass, money, duration and counts survive that test — two 3kg birds are
 * 6kg of bird, and `_g`, `_kg`, `_ms`, `_usd`, `_count` are all deliberately
 * absent below. A length, a temperature, a speed and an angle fail it, and
 * those are what this pattern catches.
 */
const UNIT_NAME = /(^|[\s_-])(mm|cm|km|nm|um|mi|in|ft|yd|deg|rad|px|pt|em|rem|bpm|rpm|mph|kph|kmh|knots|psi|hz|khz|mhz|ghz)$/i;
/**
 * The same test again, by WORD rather than by unit suffix. `proton_speed` wears
 * no unit and reads as none of the rate words, so it cleared both screens and a
 * progress ring announced "false takes 56% of the 1.9M proton_speed" — the
 * added-up speed of three thousand solar-wind readings.
 *
 * Everything listed is a property OF one thing rather than an amount of it, so
 * two of them never add to a third of the same kind. Weight, mass, duration,
 * money and counts are again deliberately absent.
 */
const INTENSIVE_NAME = /(^|[\s_-])(speed|velocity|temp|temperature|pressure|density|humidity|altitude|elevation|depth|length|width|height|diameter|radius|angle|bearing|heading|latitude|longitude|lat|lon|lng|mag|magnitude|richter|ph|abv|bmi|iq|rank|rating|quality|grade|level|percentile|stars|position|place)([\s_-]|$)/i;
const perItemMeasure = (f) => RATE_NAME.test(String(f.name))
  || UNIT_NAME.test(String(f.name))
  || INTENSIVE_NAME.test(String(f.name));
const sizeColumn = (p, exclude = []) => {
  const q = quants(p, exclude);
  return q.find((f) => !perItemMeasure(f)) || q[0] || null;
};
...
export const GALLERY = [
  ...
  },
  {
    ...
    fit(p) {
      ...
      // A BUMP HAS TO SHOW ITS STEPS. The series cap has always been here; the
      // period cap was not, so a year of daily signups was accepted and drew 357
      // columns as a dense vertical hatch in which no crossing can be seen —
      // which is the whole reading the form exists for. Same rule the rest of
      // this file applies: if a card cannot show enough, do not recommend it.
      if (distinct(x) > LIMITS.bumpPeriods) {
        return no(`"${x.name}" has ${distinct(x)} periods and a bump draws a column at each one — `
          + `past about ${LIMITS.bumpPeriods} the crossings are closer together than the lines are `
          + 'thick, so the overtaking this form exists to show cannot be seen. Aggregate to a coarser '
          + 'period, or use a line chart, which reads a long run of values well');
      }
  ...
  {
    slug: 'conv-slope', tier: 'Conventional', title: 'Slope chart', module: 'slope',
    build: 'trace', rest: 'peak',
    gallery: 'Two moments, and who overtook whom',
    answers: 'Who moved between these two points, and did the order change?',
    data: 'A series column, a number, and a period column — the first and last periods are the two ends.',
    honesty: 'Only the two ends are measured; the segment between them is a connector and claims nothing about the path. The axis includes zero, because steepness is the read.',
    fit(p) {
      // THE TWO ENDS HAVE TO BE ORDERED. A temporal or ordinal column carries
      // its own order; a plain nominal one does not, and taking "the first and
      // last" of it produced readings like "Embarked on Survived, between the
      // first and last Sex" — a real sentence this rule used to emit. A nominal
      // column is admitted only at EXACTLY two values, where there is no first
      // and last to get wrong and the chart is the standard paired comparison.
      const t = first(p, [T.TEMPORAL, T.ORDINAL], [], (f) => distinct(f) >= 2)
        || first(p, [T.QUANT], [], (f) => sequential(p, f))
        || first(p, [T.NOMINAL], [], (f) => distinct(f) === 2);
      if (!t) {
        const many = first(p, [T.NOMINAL], [], (f) => distinct(f) > 2);
        return no(many
          ? `needs an ordered column to slope between — "${many.name}" has ${distinct(many)} values in `
            + 'no particular order, and "the first and last" of an unordered column is not a comparison'
          : 'needs a period column with at least 2 distinct values to slope between');
      }
      const paired = t.type === T.NOMINAL;
      const c = cats(p, 2, 24, [t.name])[0];
      if (!c) return no(`needs a series column with 2 to 24 values to draw a line each — `
        + `"${t.name}" is being used for the two periods`);
      // An evenly-spaced integer column is an AXIS, not a height — binding a
      // year to the value would slope every series along the same diagonal.
      const v = first(p, [T.QUANT], [], (f) => !sequential(p, f));
      if (!v) return no('needs a numeric column to give each end its height');
      const skipped = distinct(t) - 2;
      return yes(`"${c.name}" on "${v.name}", `
        + (paired ? `compared across the two values of "${t.name}"`
          : `between the first and last "${t.name}"`)
        + (skipped > 0 ? ` (${skipped} intermediate ${skipped === 1 ? 'period' : 'periods'} passed over)` : ''),
        { category: c.name, value: v.name, series: t.name },
        distinct(t) === 2 ? 'strong' : 'possible');
    },
  },
  {
  ...
  },
  {
    ...
    fit(p) {
      ...
      if (perItemMeasure(v)) {
        return no(`"${v.name}" reads as a rate or a per-item measurement, and those do not sum to a `
          + 'whole — a pie of them would show a total that does not exist. Use a bar chart');
      }
  ...
  {
    slug: 'conv-gauge', tier: 'Conventional', title: 'Progress rings', module: 'gauge',
    build: 'ring', rest: 'peak',
    gallery: 'One ring each, one shared whole',
    answers: 'What share of the total does each one hold?',
    data: 'A category column with a handful of values, and a number that genuinely sums to a whole.',
    honesty: 'Every ring divides the same total at the same radius, so the arcs compare directly. The number is printed inside each ring, because angle is the weak half of the encoding.',
    fit(p) {
      const c = cats(p, 2, 40)[0];
      if (!c) return no('needs a category column to give each ring its share');
      // The same part-to-whole refusals the pie makes, for the same reasons —
      // a ring is a pie slice that has been given its own circle, and none of
      // what makes a share meaningless is fixed by moving it.
      if (distinct(c) > 8) {
        return no(`"${c.name}" has ${distinct(c)} values and past about 8 each ring is too small to `
          + 'hold its own number, which is the one thing this form promises — a bar chart compares '
          + 'this many by length');
      }
      const v = sizeColumn(p);
      if (!v) return no('needs a numeric column whose values add up to a whole');
      // The BROAD screen, not just RATE_NAME: sizeColumn falls back to the first
      // numeric column when every candidate is unsummable, so a form that
      // divides a whole has to check what it was actually handed.
      if (perItemMeasure(v)) {
        return no(`"${v.name}" reads as a rate or a per-item measurement, and those do not sum to a `
          + 'whole — a ring of them would show a share of a total that does not exist. Use a bar chart');
      }
      const e = extent(p, v.name);
      if (e && e.min < 0) {
        return no(`"${v.name}" goes negative and an arc has no way to sweep minus eight percent — `
          + 'use a bar chart, which can sit below its baseline');
      }
      return yes(`each "${c.name}" as its share of the total "${v.name}"`,
        { category: c.name, value: v.name }, distinct(c) <= 6 ? 'strong' : 'possible');
    },
  },
  {
  ...
  },
  {
    ...
    fit(p) {
      const x = ordered(p);
      if (!x) return no('needs a date column, or a numeric column that is genuinely an ordering like a year');
      // An area is a SUM accumulated from zero, so the same screen a ring or a
      // pie applies belongs here — this used to read RATE_NAME alone and stacked
      // the running total of a leaderboard "rank" as if it were a quantity.
      const y = first(p, [T.QUANT], [x.name], (f) => !perItemMeasure(f));
      if (!y) {
  ...
  },
  {
    ...
    fit(p) {
      ...
      // Every rectangle here is an AREA, and an area is a sum — so the value is
      // screened exactly as sizeColumn screens it elsewhere.
      const v = sizeColumn(p);
  ...
  {
    slug: 'unc-beeswarm', tier: 'Unconventional', title: 'Beeswarm', module: 'beeswarm',
    build: 'rain', rest: 'peak',
    gallery: 'Every observation, none of them hidden',
    answers: 'What does the whole distribution look like, row by row?',
    data: 'A numeric column, and optionally a category to give each group its own row.',
    honesty: 'Every bound row is a dot and nothing is binned or averaged. The offset away from each row centre is collision avoidance and carries no value.',
    fit(p) {
      const v = first(p, [T.QUANT], [], (f) => !sequential(p, f));
      if (!v) return no('needs a numeric column to place the dots along');
      const n = nRows(p);
      if (n < 12) {
        return no(`${n} rows is too few to need a swarm — under about 12 a dot strip shows the same `
          + 'values with nothing to resolve');
      }
      // Past this the dots are smaller than the gaps between them and the
      // picture is a texture, which is the hexbin's job rather than this one.
      if (n > 3000) {
        return no(`${n} rows would draw ${n} dots smaller than the space between them — a hexbin or `
          + 'a contour shows this density honestly');
      }
      const c = cats(p, 2, 12)[0];
      return yes(c
        ? `every "${v.name}" drawn, one row per "${c.name}"`
        : `all ${n} values of "${v.name}" drawn as one swarm`,
        { value: v.name, category: c ? c.name : undefined }, 'strong');
    },
  },
  {
  ...
  {
    slug: 'unc-radar', tier: 'Unconventional', title: 'Radar', module: 'radar',
    build: 'bloom', rest: 'walk',
    gallery: 'A named shape per contender',
    answers: 'How does each one score across the same set of measures?',
    data: 'LONG: a series column, a measure column, and the value — one row per series per measure.',
    honesty: 'Reach along each spoke is the encoding. Enclosed area is not, because area grows with the square of reach and changes with an axis ORDER that is a layout choice.',
    fit(p) {
      // WHICH COLUMN IS THE SPOKE AND WHICH IS THE SHAPE is not answerable from
      // column order, and getting it backwards draws a readable picture of the
      // wrong question — four capabilities orbiting each vendor rather than four
      // vendors compared across capabilities. The spokes are the column with
      // MORE distinct values: more measures to compare on, fewer things being
      // compared, which is what the form is for.
      const pair = cats(p, 2, 24);
      if (pair.length < 2) {
        return no('needs two category columns — one naming the measures (the spokes), one naming '
          + 'the series (the shapes)');
      }
      const ordered = [...pair].sort((a, b) => distinct(b) - distinct(a));
      const axis = ordered[0];
      const series = ordered.find((f) => f !== axis && distinct(f) >= 2 && distinct(f) <= 6);
      if (distinct(axis) < 3 || distinct(axis) > 12) {
        return no(`"${axis.name}" would stand ${distinct(axis)} spokes — a radar needs 3 to 12, `
          + 'because under 3 there is no shape and past 12 the spokes are closer than the labels');
      }
      if (!series) {
        const wide = ordered.find((f) => f !== axis && distinct(f) > 6);
        return no(wide
          ? `needs a series column with at most 6 values so the shapes stay separable — "${wide.name}" has ${distinct(wide)}`
          : 'needs a second category column naming the series, one shape each');
      }
      const v = first(p, [T.QUANT], [], (f) => !sequential(p, f));
      if (!v) return no('needs a numeric column to give each spoke its reach');
      return yes(`"${series.name}" compared across "${axis.name}" on "${v.name}"`,
        { series: series.name, axis: axis.name, value: v.name },
        distinct(series) <= 4 ? 'strong' : 'possible');
    },
  },
  {
  ...
  {
    slug: 'exp-cube', tier: 'Experimental', title: 'Data cube', module: 'data-cube',
    build: 'emerge', rest: 'peak',
    gallery: 'Three measures, turned by hand',
    answers: 'How do three measures trade off against each other at once?',
    data: 'Three numeric columns. Add a label for the points and a category to group them.',
    honesty: 'Orthographic projection, so a point at the back is drawn at the same scale as one at the front and position stays true to value. Occlusion is resolved by turning it, and snapping to a face collapses the third axis into a true flat scatter.',
    fit(p) {
      // The three axes must be MEASUREMENTS. An evenly-spaced integer column is
      // an ordering, and standing a year up as a spatial axis invents a
      // dimension the table does not have.
      const q = quants(p).filter((f) => !sequential(p, f));
      if (q.length < 3) {
        return no(`needs three numeric columns to stand up as axes — found ${q.length}`);
      }
      if (nRows(p) < 3) return no(`needs at least 3 rows to make a cloud (found ${nRows(p)})`);
      const label = cats(p, 2)[0] || null;
      const group = cats(p, 2, 8, label ? [label.name] : [])[0] || null;
      return yes(`"${q[0].name}", "${q[1].name}" and "${q[2].name}" as three axes`
        + (group ? `, grouped by "${group.name}"` : ''),
        {
          x: q[0].name, y: q[1].name, z: q[2].name,
