...
//
// Shared runtime for Deevee PORTABLE MODULES — the embeddable form of a
...
//   - House hexes baked into JS                     -> CSS custom properties
//
...
export const THEME_DEFAULTS = {
  ...
  accent: '#a82d18',
  // Measured, not picked: every entry clears 4.5:1 as text on the paper, and
  // no two land within a perceptual distance of 45 under protanopia,
  // deuteranopia or tritanopia. themes/light.css restates the numbers.
  cat: ['#2f6fd0', '#0f2a4d', '#a82d18', '#0b5c45', '#8a6a00', '#6b7280'],
...
.${rootClass}{
  ...
  /* THE FIELD MARK — the colour of every mark that is NOT the emphasis, in a
     ...
     paper but had no way to say what the chart FIELD is made of. The House
     theme's own law is "one blue/ink family for the field, Accent only where
...
/* COMPACT: an imposed box too short to hold the full apparatus.
   A 360x260 dashboard cell was giving the chart a sliver — headline, dek and a
   five-line source note took the box and the plot got what was left, which is
   the opposite of the priority a chart card should have. So in a SHORT box the
   dek goes (it restates what the headline and the marks already say) and the
   source note is clamped to two lines, with its full text kept on the element's
   title so nothing disclosed is actually lost. The headline, the plot and the
   source all survive; only their share of the box changes.
   Applied from a measured height in observeSize rather than a container query,
   because container-type:size would force a definite block size on every host
   including the flowing-page one, where it collapses the frame. */
.${rootClass}.vf-compact{ padding: 0.8rem 0.9rem; }
.${rootClass}.vf-compact .vf-dek{ display: none; }
.${rootClass}.vf-compact .vf-headline{ font-size: clamp(0.95rem, 4.2cqw, 1.15rem); }
.${rootClass}.vf-compact .vf-plot{ min-height: 0; }
.${rootClass}.vf-compact .vf-source{
...
/* A STATIC key entry. The swatch is sized with width/height, which does
   nothing to an inline span — it needs a flex or block parent. The interactive
   key below gets that from its own <button>; a plain <li> did not, so the
   names rendered with no colours beside them at all, which is worse than no
   key because it reads as a finished legend. */
.${rootClass} .vf-legend li{
  display: inline-flex;
  align-items: center;
  gap: 0.45rem;
  font-family: var(--_fl);
...
export function buildFrame(el, config, opts) {
  ...
  // The compact state clamps this to two lines. Carrying the same text on the
  // title keeps every disclosure reachable rather than merely cropped — a
  // truncated honesty note that cannot be recovered is a lost one.
 ...
 */
// Below this a plot box cannot hold a chart at all, and honouring it exactly
// would mean drawing marks a couple of pixels tall. The scale-to-fit the old
// code applied everywhere is the right answer HERE and only here.
const FLOOR_HEIGHT = 120;

// Below this the frame's copy is competing with its own chart for the box.
...
export function observeSize(el, plot, render, { aspect = 0.52, minHeight = 190, fit = 'aspect' } = {}) {
  ...
  const apply = () => {
    ...
    // aspect rule only when the host imposes NO height (a flowing page).
    //
    // The distinction matters more than it looks. This used to read
    // `box > minHeight ? box : Math.max(minHeight, width * aspect)`, which
    // treated "a real height, but a tight one" as "no height at all" — so a
    // 1116x193 plot box was handed a 1116x558 viewBox, and `svg{height:100%}`
    // then scaled the finished drawing to 35% to make it fit. Every label came
    // out a third of its intended size, the wider the embed the worse it got,
    // and nothing in the picture said it had happened. A short box is a real
    // constraint: draw honestly INTO it rather than drawing big and shrinking.
    let height;
    if (fit === 'height') {
      const box = Math.round(plot.clientHeight || 0);
      height = box > 0 ? Math.max(box, FLOOR_HEIGHT) : Math.max(minHeight, Math.round(width * aspect));
    } else {
      height = Math.max(minHeight, Math.round(width * aspect));
    }

    // A short imposed box gets the compact treatment; a flowing page never does,
    // because there the host has not constrained anything and the full
    // apparatus is what was asked for.
    if (fit === 'height') {
      const boxH = Math.round(el.clientHeight || 0);
      el.classList.toggle('vf-compact', boxH > 0 && boxH < COMPACT_BELOW);
    }
 ...
 * the House ramp deliberately includes Accent at cat-3, so a piece with three
 ...
 * headline names.
 *
 * The house ramp deliberately contains the accent (Accent sits at
...
const RESTS = {
   ...
   * the gallery's "Accent where the story peaks" law: emphasis stays alive
